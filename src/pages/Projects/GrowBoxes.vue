<template>
  <section class="flex items-center">
    <div
      class="relative w-[100px] h-[100px] sm:w-[268px] sm:h-[268px]"
      ref="wrapper"
    >
      <div
        role="button"
        ...
        :class="[
          shape === 'circle'
            ? 'rounded-full w-4 h-4 sm:w-6 sm:h-6'
            : 'rounded-none w-4 h-4 sm:w-8 sm:h-8',
          `${lineColor} absolute w-4 h-4 sm:w-6 sm:h-6 h-calc(100%/100)`,
        ]"
        :style="{
          top: entity.x + 'px',
          left: entity.y + 'px',
      ...
      x_yArray.forEach(function (entity) {
        ...
        setTimeout(function () {
          if (checked === true) {
            ...
            props.shape === 'circle'
              ? (element.className =
                  `absolute rounded-full tile w-4 h-4 sm:w-4 sm:h-4 grow ` +
                  endColor)
              : (element.className =
